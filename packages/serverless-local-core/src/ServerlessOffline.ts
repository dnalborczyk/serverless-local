...
import serverlessLog, { logWarning, setLog } from '@serverless-local/logger'
...
// import type Http from '@serverless-local/event-http'
// import type Schedule from '@serverless-local/event-schedule'
// import type WebSocket from '@serverless-local/event-websocket'
import {
  HttpCreateOptions,
  LambdaCreateOptions,
  ScheduleCreateOptions,
  WebSocketCreateOptions,
} from '@serverless-local/types'
...
export default class ServerlessOffline implements Plugin {
  ...
  private async _createLambda(
    ...
    const { default: Lambda } = await import('@serverless-local/handler-lambda')
  ...
  private async _createHttp(
    ...
    const { default: Http } = await import('@serverless-local/event-http')
  ...
  private async _createSchedule(
    ...
    const { default: Schedule } = await import(
      '@serverless-local/event-schedule'
    )
  ...
  private async _createWebSocket(
    ...
    const { default: WebSocket } = await import(
      '@serverless-local/event-websocket'
    )
