...
import serverlessLog, { logRoutes } from '@serverless-local/logger'
import {
  debugLog,
  detectEncoding,
  jsonPath,
...
export default class HttpServer {
  ...
  createRoutes(functionKey: string, httpEvent, handler: string): void {
  ...
  // Bad news
  private _reply500(response, message, error) {
  ...
  private _getArrayStackTrace(stack: string): string[] {
    if (!stack) {
      return null
    }
  ...
  private _injectLastRequest(): void {
